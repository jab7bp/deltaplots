...
Double_t ymax_dxdy = 2.5;
	...
	h_theta_pq_p = new TH1D("h_theta_pq_p", Form("Theta pq for proton - SBS%i %i, %s; theta_pq_p (rad);", kine, sbsfieldscale, run_target.Data()), 5000, 0.0, 5.0);
	...
	if( kine == 8 ){
		...
		if( use_parsed ){
			master_cut_vec = {
					...
					// Form("bb.tr.p[0]>%f", 1.10*lookup_parsed_cut(run_target.Data(), kine, sbsfieldscale, "SH_PS_mean") ),
					// "sbs.hcal.e>0.05",
					// Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))>(%f)", 0.80),
					// Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))<(%f)", 1.25),
					// "bb.ps.e+bb.sh.e>2.5", //-->bb.tr.p[0]
					// "((e.kine.W2)>0.5)&&((e.kine.W2)<1.40)",
					// "bb.tr.p[0]>3.05",
					// // Form("bb.ps.e>%f", lookup_parsed_cut(run_target, kine, sbsfieldscale, "PS_min")),
...
//Calculate theta pq variables
	...
	//Expected proton direction
		...
		TVector3 Proton_Direction = (HCAL_pos + Proton_Deflection*HCAL_zaxis - vertex).Unit();
		...
		double ePeak_ep = (E_beam_final)/( 1 + (2*E_beam_final/Mp)*pow( sin( BB_theta/2 ), 2 ) );
		double phel_p = sqrt( pow( nu_recon, 2 ) + 2*Mp*nu_recon );

		theta_pq_p = acos( (E_beam_final - ePeak_ep*cos(BB_theta) )/(phel_p) );
	...
	//---------------------------
		...
		// 			// continue;
		// 		}
		// 		if( theta_pq_p < theta_pq_n_thresh){
		// 			// continue;
		// 		}
...
//-----------------------------------------------------
	...
	//START OF LOOP FOR BEST CLUSTER SELECTION:
		...
		//We want the cluster which minimizes the ADCtime
			if( abs(hcal_clus_sel_ADCtime) < clus_sel_atime ){
				//Will save/overwrite the clus_sel with minimum clus_sel_atimediff
				clus_sel_atime = abs(hcal_clus_sel_ADCtime);
		...
		//We want cluster which minimized difference between coincidence times
			...
			if( abs(hcal_clus_sel_ADCtime_diff) < clus_sel_atimediff ){
				clus_sel_atimediff = abs(hcal_clus_sel_ADCtime_diff);
				clus_sel_dx_atimediff_simple = clus_sel;
			}
	...
	/// ADC Coincidence timing check. Save the clusters that pass and then assess the clusters' energies
		...
		//NOW, we want to select the highest energy cluster in those that have passed the ADC timing window
		/////THIS Happens after the cluster loop --> So we can compare all the clusters that passed
			//ADC Coin timing:
			...
			//IF there is only one passing cluster then let's just use that.
			if( ADC_coin_pass_cnt == 1 ){
				clus_sel_dx_atime_and_maxE = hcal_e_index_ADC_coin_passed[0];
				eClusMaxIndex = hcal_e_index_ADC_coin_passed[0];
			}

			//Otherwise, lets scan through the vector and find the maximum
			else if( ADC_coin_pass_cnt > 1 && hcal_e_ADC_coin_passed.size() > 1 && hcal_e_index_ADC_coin_passed.size() > 1 ){
				
				//Let us look through the passing clusters and find the highest energy cluster and corresponding clus_sel
				//In case some energies have the same value (very very very unlikely) we pick the matching energy with lowest index
				double eClusMax = 0.0;
				
				for( size_t e = 0; e < hcal_e_ADC_coin_passed.size(); e++ ){
					if( hcal_e_ADC_coin_passed[e] > eClusMax ){
						eClusMax = hcal_e_ADC_coin_passed[e];
						//This gives us the value of "clus_sel" stored for the corresponding max E
						eClusMaxIndex = hcal_e_index_ADC_coin_passed[e];
					}
				}	

			//Now, we place pick out the clus_sel corresponding to max energy. 
		        clus_sel_dx_atime_and_maxE = eClusMaxIndex;
			//Now we can apply this and use it to select "best clusters" to use:
		//SELECTION: Max energy cluster of those that pass the timing window:    
			}

			h_eclus_sel->Fill(eClusMaxIndex);
