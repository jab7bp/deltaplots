...
#include "/w/halla-scshelf2102/sbs/jboyd/include/SetupSystematicVariable.h"
...
bool apply_adc_diff_time_cut = true;
bool apply_ADC_cut = true;
...
double Ep_sig_mult, SH_PS_sig_mult, Ep_min, Ep_max, SH_PS_min, HCal_clus_e_min, fcut_mult;
...
double w2_mult = 1.5;
double w2_mult_min = 1.5, w2_mult_max = 1.5, systematic_invMass_min = 1.5, systematic_invMass_max = 1.5;


double W_cut_lower_bound = 2.0, W_cut_upper_bound = 2.0;

TString systematic_varVal_str = "";

void dxdy_systematics(int kine, int sbsfieldscale, TString run_target, TString systematic_var_str, double val_min = 2.0, double val_max = 2.0 ){

	gROOT->SetBatch(kTRUE);

	//Before we can use the "SetupSystematicVariable" function we need to first set the apprpriate variables in the systematic variable class:
	syst_vars.val_min = val_min;
	syst_vars.val_max = val_max;
	syst_vars.systematic_var_str = systematic_var_str;
	SetupSystematicVariable( syst_vars );
	...
	sbsfieldscale_Frac = sbsfieldscale/100.0;
	...
	outfile = new TFile(Form("systematics_rootfiles/SBS%i/%s/%s_systematics_%s_SBS%i_mag%i_varVal_FcutMod_%s.root", kine, systematic_var_str.Data(), systematic_var_str.Data(), run_target.Data(), kine, sbsfieldscale,  systematic_varVal_str.Data()), "RECREATE");
	// outfile = new TFile(Form("rootfiles/%s_SBS%i_mag%i%s_dxdy%s_elastics_only_bestCluster_%i%s%s_added_Wplots_03_01_2024_oldParsed.root", run_target.Data(), kine, sbsfieldscale, pq_cut_String.Data(), parsed_sel_string.Data(), use_best_cluster,  best_cluster_OnOff_indicated.Data(), ADC_timing_string.Data() ), "RECREATE");		
...
Double_t ymax_dxdy = 2.5;
	...
	h_HCal_xy_expected_std = new TH2D("h_HCal_xy_expected_std", Form("Expected HCal x and y, raw - SBS%i %i, %s;y_{HCal} (m); x_{HCal} (m)", kine, sbsfieldscale, run_target.Data()), nBins_x_dxdy, xmin_dxdy, xmax_dxdy, nBins_y_dxdy, ymin_dxdy, ymax_dxdy);
	h_HCal_xy_expected_fcut = new TH2D("h_HCal_xy_expected_fcut", Form("Expected HCal x and y, Fcut - SBS%i %i, %s;y_{HCal} (m); x_{HCal} (m)", kine, sbsfieldscale, run_target.Data()), nBins_x_dxdy, xmin_dxdy, xmax_dxdy, nBins_y_dxdy, ymin_dxdy, ymax_dxdy);
	...
	if( kine == 9 ){
		hcal_height = -0.35021355; //-0.36091616; //-0.36307351; // -0.36249082 //-0.35863270 //more negative puts the dx_n more negative
	}
	...
	W2_mean = lookup_parsed_cut(run_target, kine, sbsfieldscale, "W2");
	W2_sigma = lookup_parsed_cut(run_target, kine, sbsfieldscale, "W2_sigma");

	if( systematic_var_str == "inv_mass_fixed_mean_vary_sigma_mult" || systematic_var_str == "inv_mass_fixed_moving_range" || systematic_var_str == "inv_mass_fixed_min_vary_max" ){
		vary_on_invMass = true;
	}
	...
	if( systematic_var_str == "inv_mass_fixed_mean_vary_sigma_mult"  ){
		...
		W_cut_lower_bound = (Wcut_select_mean - w2_mult_min*Wcut_select_sigma );
		W_cut_upper_bound = (Wcut_select_mean + w2_mult_max*Wcut_select_sigma );
	...
	cout << endl;
	cout << "W_cut_lower_bound: " << W_cut_lower_bound << " ---- W_cut_upper_bound: " << W_cut_upper_bound << endl;
	cout << "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*" << endl;
	...
	if( multi_run && use_parsed ){
		rootfile_dir = "/volatile/halla/sbs/jboyd/analysis_rootfiles/jboyd_parsed";
		cout << "--------------------------------------" << endl;
		cout << "     Multi-run USING PARSED FILES " << endl;
		cout << "--------------------------------------" << endl;
		cout << "Rootfile dir: " << rootfile_dir.Data() << endl;
...
//------------
		...
		TC->Add(Form("%s/gmn_parsed_%s_SBS%i_mag%i.root", rootfile_dir.Data(), run_target.Data(), kine, sbsfieldscale));
		// TC->Add(Form("%s/gmn_parsed_LD2_SBS9_mag70_old.root", rootfile_dir.Data()));
//-------------
...
//Now getting fiducial cut values from beam_variables.h
	...
	if( systematic_var_str == "SH_PS_sigma_mult"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING SH_PS sigma multiplier BEFORE Master Cut Vec: " << endl;
		cout << "Standard SH_PS_sigma_mult = " << SH_PS_sig_mult << endl;
		double old_SH_PS_min = lookup_parsed_cut(run_target, kine, sbsfieldscale, "SH_PS_mean") - SH_PS_sig_mult*lookup_parsed_cut(run_target, kine, sbsfieldscale, "SH_PS_sigma")/2.0;
		cout << "Standard SH_PS min: " << old_SH_PS_min << endl;
		cout << endl;

		cout << "NEW SH_PS sigma multiplier: " << syst_vars.SHPS_sigma_mult_min << endl;
		SH_PS_sig_mult = syst_vars.SHPS_sigma_mult_min;
		double new_SH_PS_min = lookup_parsed_cut(run_target, kine, sbsfieldscale, "SH_PS_mean") - SH_PS_sig_mult*lookup_parsed_cut(run_target, kine, sbsfieldscale, "SH_PS_sigma")/2.0;
		cout << "NEW SH_PS min: " << new_SH_PS_min << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}
	if( systematic_var_str == "Ep_min"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING Ep_min BEFORE Master Cut Vec: " << endl;
		double old_Ep_min = Ep_min;
		cout << "Standard Ep_sig_min = " << old_Ep_min << endl;
		cout << "NEW Ep_min: " << syst_vars.Ep_min << endl;
		Ep_min = syst_vars.Ep_min;
		double new_Ep_min = Ep_min;
		cout << "NEW Ep min: " << new_Ep_min << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}
	if( systematic_var_str == "PS_min"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING PS_min BEFORE Master Cut Vec: " << endl;
		cout << "Standard PS_min = " << PS_min << endl;

		PS_min = syst_vars.PS_min;

		cout << "NEW PS min: " << PS_min << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}
	if( systematic_var_str == "HCal_clus_e_min"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING HCal_clus_e_min BEFORE Master Cut Vec: " << endl;
		cout << "Standard HCal_clus_e_min = " << HCal_clus_e_min << endl;

		HCal_clus_e_min = syst_vars.HCal_clus_e_min;

		cout << "NEW HCal_clus_e_min: " << HCal_clus_e_min << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}
	if( systematic_var_str == "gem_track_nhits_min"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING gem_track_nhits_min BEFORE Master Cut Vec: " << endl;
		cout << "Standard gem_track_nhits_min = " << gem_track_nhits_min << endl;

		gem_track_nhits_min = syst_vars.gem_track_nhits_min;

		cout << "NEW HCal_clus_e_min: " << gem_track_nhits_min << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}
	if( systematic_var_str == "fcut_mult"){
		...
		cout << "oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
		cout << "	MODIFYING fcut_mult BEFORE Master Cut Vec: " << endl;
		cout << "Standard fcut_mult = " << fcut_mult << endl;

		fcut_mult = syst_vars.fcut_mult;

		cout << "NEW fcut_mult: " << fcut_mult << endl;
		cout << "#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-" << endl;
	}	
	...
	if( kine == 4 ){
		master_cut_vec = {
				...
				Form("bb.gem.track.nhits[0]>%i", gem_track_nhits_min),
				"bb.tr.n==1",
				Form("bb.ps.e>%f", PS_min),
				// Form("bb.tr.p[0]>%f", 1.10*lookup_parsed_cut(run_target.Data(), kine, sbsfieldscale, "SH_PS_mean") ),
				Form("sbs.hcal.e>%f", HCal_clus_e_min),
	...
	if( kine == 9 ){
		if( use_parsed ){
			master_cut_vec = {
					...
					"bb.tr.n==1",
					Form("bb.ps.e>%f", PS_min),
					Form("(bb.sh.e+bb.ps.e)>%f", SH_PS_min), //TYPICALLY OFF
					Form("((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))>(%f)", Ep_min)
				};
		}
		if( !use_parsed ){
...
// --------------------------------------
	...
	// Coincidence timing cut and vertex cut to resolve W well
		...
		if( ( (W_cut_lower_bound) <= W2) && ( W2 <= (W_cut_upper_bound) ) ){
	...
	//FIDUCIAL Cut
		...
		//For testing out the effect of fcut_mult.... let's have a section to always look at 2*fcut_mult and 3*fcut_mult
			if( ((y_expected_HCal - 2.0*fcut_mult*dy_p_sigma) > hcal_y_fmin) && ((y_expected_HCal + 2.0*fcut_mult*dy_p_sigma) < hcal_y_fmax) && ((x_expected_HCal - dx_pn_max - 2.0*fcut_mult*dx_p_sigma) > hcal_x_fmin) && ((x_expected_HCal + dx_pn_max + 2.0*fcut_mult*dx_p_sigma) < hcal_x_fmax) ){
				if(( (W_cut_lower_bound) < W) && ( W < (W_cut_upper_bound) )){
